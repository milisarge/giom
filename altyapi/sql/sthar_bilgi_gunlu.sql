...
set @stkod := '@stkod@';
select 
DATE_FORMAT(u.tarih, '%d.%m.%Y') as tarih,u.fiþno,u.hareket_tipi,u.nf,u.bf ,u.giriþ,u.çýkýþ,u.bakiye,u.ack,u.sthkod
from(
select DATE_FORMAT(t.tarih, '%Y.%m.%d') as tarih,t.fiþno,t.hareket_tipi,round(t.kdvsizbf,3) as nf,round(t.kdvlibf,3) as bf ,round(t.giris,3) as giriþ,round(t.cikis,3) as çýkýþ,
...
where
u.tarih  between date_add(now(),interval -@gun@ day) and now()